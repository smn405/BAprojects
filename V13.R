...
loan.dates$recoveries <- NULL
loan.dates$total_pymnt <- NULL
...
loan.bins$loan_amnt_band <- cut(loan.bins$loan_amnt, c(0,5000,10000,15000,20000,25000,30000,35000))
...
loan.bins$dti_band <- cut(loan.bins$dti, c(-1,0,10,20,30,40,50,60,673))
...
loan.bins$collections_12_mths_ex_med_band <- cut(loan.bins$collections_12_mths_ex_med, c(-1,0,1,2,3,4,20))
...
write.csv(rfImp.T, "factors7.csv")
...
band3 <- as.numeric(rf.probs$`1`)
scoreband3 <- cut(band,3)
barplot(prop.table(table(scoreband3)))
...
write.csv(gbmImp.T, "factorsGBM.csv")
...
gbm.plot <- plot(roc(test.df$default, gbm.probs[,2]))

band <- as.numeric(gbm.probs$`1`)
scoreband <- cut(band,10)
barplot(prop.table(table(scoreband)))

band3 <- as.numeric(gbm.probs$`1`)
scoreband3 <- cut(band,3)
barplot(prop.table(table(scoreband3)))
...
fitControl.gbm2 <- trainControl(method = "repeatedcv",
                               ...
                               repeats = 4,
                               sampling = "up")
gbm2 <- train(train.df[,predictorNames], train.df[,outcomeName],
             method = 'gbm',
             trControl = fitControl.gbm2)

library(gbm)
gbmImp2 <- varImp(gbm2, scale = TRUE)
gbmImp2
plot(gbmImp2, top = 20)

gbmImp2.T <- as.data.frame(gbmImp2$importance)
write.csv(gbmImp2.T, "factorsGBM-Up.csv")

gbm2.predict <- predict(gbm2, test.df[,predictorNames], type = "raw")
confusionMatrix(gbm2.predict, test.df[,outcomeName], positive = "1")

gbm2.probs <- predict(gbm2, test.df[,predictorNames], type = "prob")
gbm2.plot <- plot(roc(test.df$default, gbm2.probs[,2]))

band <- as.numeric(gbm2.probs$`1`)
scoreband <- cut(band,10)
barplot(prop.table(table(scoreband)))
...
fitControl.gbm3 <- trainControl(method = "cv",
                                number = 10)
gbm3 <- train(train.df[,predictorNames], train.df[,outcomeName],
              method = 'gbm',
              trControl = fitControl.gbm3)

library(gbm)
gbmImp3 <- varImp(gbm3, scale = TRUE)
gbmImp3
plot(gbmImp3, top = 20)

gbmImp3.T <- as.data.frame(gbmImp3$importance)
write.csv(gbmImp2.T, "factorsGBM-CV.csv")

gbm3.predict <- predict(gbm3, test.df[,predictorNames], type = "raw")
confusionMatrix(gbm3.predict, test.df[,outcomeName], positive = "1")

gbm3.probs <- predict(gbm3, test.df[,predictorNames], type = "prob")
gbm3.plot <- plot(roc(test.df$default, gbm3.probs[,2]))

band <- as.numeric(gbm3.probs$`1`)
scoreband <- cut(band,10)
barplot(prop.table(table(scoreband)))
...
fitControl.gbm4 <- trainControl(method = "repeatedcv",
                                ...
                                repeats = 4,
                                sampling = "down")
gbm4 <- train(train.df[,predictorNames], train.df[,outcomeName],
              method = 'gbm',
              trControl = fitControl.gbm4)

library(gbm)
gbmImp4 <- varImp(gbm4, scale = TRUE)
gbmImp4
plot(gbmImp4, top = 20)

gbmImp4.T <- as.data.frame(gbmImp4$importance)
write.csv(gbmImp4.T, "factorsGBM-Down.csv")

gbm4.predict <- predict(gbm4, test.df[,predictorNames], type = "raw")
confusionMatrix(gbm4.predict, test.df[,outcomeName], positive = "1")

gbm4.probs <- predict(gbm4, test.df[,predictorNames], type = "prob")
gbm4.plot <- plot(roc(test.df$default, gbm4.probs[,2]))
