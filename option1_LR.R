...
index <- sample(1:nrow(loan.df),(.05)*nrow(loan.df))
part <- loan.df [index, ]
#prop.table(table(part.dummified$default))


part$id <- NULL
part$funded_amnt_band <- NULL
part$funded_amnt_inv_band <- NULL
part$out_prncp_inv_band <- NULL
part$total_pymnt_inv_band <- NULL
part$total_rec_prncp_band <- NULL
part$total_rec_int_band <- NULL
part$pmnt_months_band<- NULL 
part$pmnt_diff_band<- NULL 

dataDummy <- dummyVars("~.", data = part, fullRank = F)
data.dummified <- as.data.frame(predict(dataDummy, part))
data.dummified$default <- as.factor(data.dummified$default)
...
set.seed(1234)
split <- (.8)
index <- createDataPartition(data.dummified$default, p=split, list=FALSE)

train.df <- data.dummified[index, ]
test.df <- data.dummified[-index, ]

prop.table(table(train.df$default))
train.df$default <- as.factor(train.df$default)

# Logistics Regression
names(train.df)

#lapply(train.df, unique)


glm.fit <- glm(default~., data = train.df, family = binomial)
summary(glm.fit)
